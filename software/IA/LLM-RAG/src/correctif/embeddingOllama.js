const content = `import { Embeddings } from "@langchain/core/embeddings";
export class OllamaEmbeddings extends Embeddings {
    ...
    async _request(prompt) {
        ...
        const response = await fetch(\`\${formattedBaseUrl}/api/embeddings\`, {
            ...
            body: JSON.stringify({
                prompt: prompt.prompt || prompt.question || prompt,
                model,
...
if(fs.existsSync(path.resolve('./node_modules/@langchain/community/dist/embeddings/ollama.js'))) {
    fs.writeFile(path.resolve('./node_modules/@langchain/community/dist/embeddings/ollama.js'), content, 'utf8', err => err ? console.log(err) : '')
    console.info('\x1b[32mCorrectif appliqué avec succès !\x1b[0m')
} else console.log('\x1b[31mAucun correctif à appliquer car les modules ne sont pas trouvés\x1b[0m')