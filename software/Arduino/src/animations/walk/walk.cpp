...
void UpLeg() {
  for (int i = 0; i < sizeof(WalkUpMatrice) / sizeof(WalkUpMatrice[0]); ++i) {
    ...
    SetLeg(WalkUpMatrice[i][0], WalkUpMatrice[i][1], WalkUpMatrice[i][2], WalkUpMatrice[i][3], LML, 0);
    SetLeg(WalkUpMatrice[i][0], WalkUpMatrice[i][1], WalkUpMatrice[i][2], WalkUpMatrice[i][3], LBR, 0);
    
    //LEG
    if(i === 2 || i === 3) {

      SetLeg(WalkBottomMatrice[i-1][0], WalkBottomMatrice[i-1][1], WalkBottomMatrice[i-1][2], WalkBottomMatrice[i-1][3], LFL, 0);
      SetLeg(WalkBottomMatrice[i-1][0], WalkBottomMatrice[i-1][1], WalkBottomMatrice[i-1][2], WalkBottomMatrice[i-1][3], LMR, 0);
      SetLeg(WalkBottomMatrice[i-1][0], WalkBottomMatrice[i-1][1], WalkBottomMatrice[i-1][2], WalkBottomMatrice[i-1][3], LBL, 0);
