...
void SetLeg(float x, float z, float y, int duree, int* LEG, int address) {
  LegAngles res = Algo(x, z, y, duree);

  float angles[3];

  if (LEG == LML || LEG == LFL) {//Si c'est une patte de gauche alors il faut inverser les angles tibia et femur
    Serial.println(res.AngleTibia);
  ...
  int aaaa[] = {5, 6, 7};
  setServo(aaaa, 3, angles, address);
}
...
void Sleep() {
  for (int i = 0; i < sizeof(matrice_sleep) / sizeof(matrice_sleep[0]); ++i) {
    ...
    //Patte avant gauche
    SetLeg(matrice_sleep[i][0], matrice_sleep[i][1], matrice_sleep[i][2], matrice_sleep[i][3], LFL, 0);
