...
void setLegs(LegConfig legs[]) {
  ...
  // }
  unsigned long startTime = millis(); // Heure de début
  for(int ii = 0; ii < sizeof(legs); ++ii) {
    
    int legAdress = legs[ii].servoChannels[0];//definir l'adresse de la patte
    LegConfig leg = legs[ii];//definir la patte

    float legStartAngles[3] = {startAngles[legAdress].coxa, startAngles[legAdress].femur, startAngles[legAdress].tibia};
    float deltaAngles[3];//Calcul le delta entre l'angle de début et l'angle à atteindre
    float newAngles[3];//variable pour stocker les nouveaux angles dans le mouvement progressif

    //set deltaAngles
    deltaAngles[0] = leg.coxa - legStartAngles[0];
    deltaAngles[1] = leg.femur - legStartAngles[1];
    deltaAngles[2] = leg.tibia - legStartAngles[2];

    while (millis() - startTime < leg.duree) { // Tant que le temps écoulé est inférieur à la durée
          unsigned long currentTime = millis();
          float progress = (float)(currentTime - startTime) / leg.duree;
          ...
          for (int i = 0; i < 3; ++i) {
              if (leg.address == 1) {
                  pwm2.writeMicroseconds(legs[ii].servoChannels[i], pulseWidth(newAngles[i]));
              } else {
                  pwm.writeMicroseconds(legs[ii].servoChannels[i], pulseWidth(newAngles[i]));
              }
          }
      }
  }
